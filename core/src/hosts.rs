...
pub fn merge_hosts(remote_content: &str) -> Result<(), String> {
    ...
    if hosts_file.exists() {
        current_content = fs::read_to_string(hosts_file).map_err(|e| format!("Failed to read hosts: {}", e))?;
    }
    ...
    fs::write(hosts_file, new_content).map_err(|e| format!("Failed to write hosts (need admin rights?): {}", e))?;
...
mod tests {
    ...
    fn test_merge_hosts_existing() {
        ...
        let remote = "127.0.0.1 discord.com";
        let expected = format!("127.0.0.1 localhost\n{}\n127.0.0.1 discord.com\n{}\n", MARKER_START, MARKER_END);
        assert_eq!(merge_logic(current, remote), expected);
    }
