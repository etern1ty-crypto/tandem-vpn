...
//!
//! `sing-box.exe run -c config.json` is a plain console binary — it does not
//! implement the Windows Service Control Protocol, so `sc create`/`sc start`
//! fails with error 1053 as soon as the SCM expects a status callback (the
//! same mistake the old `winws.exe` integration made). Persistence instead
//! uses a scheduled task that runs at startup under `SYSTEM` with the
//! highest privileges — the pattern zapret's own Windows docs recommend for
//! `winws.exe` instead of `sc create`.
//!
//! sing-box provides a TUN inbound that transparently captures all system
//! traffic. `route.rules` decide, per connection, whether traffic goes out
//! `direct`, through `warp`, or through a Goida-sourced outbound — see
//! [`crate::warp`] and [`crate::goida`] for those outbound kinds.
//!
...
use crate::sys::{parse_schtasks_state, PlannedCommand, ServiceState, Sys};
...
impl EngineManager {
    ...
    /// Build the `/tr` value for `schtasks /create`: the quoted sing-box
    /// path invoking `run -c <config>`.
    ...
    /// Install (or reinstall) the engine as a startup scheduled task.
    ...
    pub fn install_service<S: Sys>(&self, sys: &S) -> Result<()> {
        if !self.binary_present() {
            return Err(Error::Other(
                "sing-box.exe is missing in the engine directory".to_string(),
            ));
        }

        // Clean any previous instance first. Ignore errors if not present.
        let _ = sys.run(&PlannedCommand::new(
            "schtasks",
            ["/end", "/tn", SERVICE_NAME],
        ));
        let _ = sys.run(&PlannedCommand::new(
        ...
        // Run it now too, so the engine is active immediately instead of
    ...
    /// Stop and remove the engine's scheduled task.
    ...
    pub fn status<S: Sys>(&self, sys: &S) -> Result<EngineStatus> {
        ...
        let service = parse_schtasks_state(&out.stdout, out.code);
        Ok(EngineStatus {
            service,
...
mod tests {
    ...
    fn install_plans_expected_commands() {
        ...
        // Create dummy binary so the present check passes (test uses MockSys for commands)
        std::fs::write(mgr.binary_path(), b"dummy").unwrap();
        let sys = MockSys::ok();
        mgr.install_service(&sys).unwrap();
        let log = sys.log();
        assert!(log
            .iter()
            .any(|c| c.starts_with("schtasks /create") && c.contains(SERVICE_NAME)));
        assert!(log.iter().any(|c| c.contains("sing-box.exe")));
        assert!(log.iter().any(|c| c.contains("run -c")));
        assert!(log.iter().any(|c| c.contains("/ru SYSTEM")));
        assert!(log
            .iter()
            .any(|c| c.starts_with("schtasks /run") && c.contains(SERVICE_NAME)));
        let _ = std::fs::remove_dir_all(mgr.install_dir());
    }
    ...
    fn remove_plans_expected_commands() {
        ...
        assert!(log
            .iter()
            .any(|c| c.starts_with("schtasks /delete") && c.contains(SERVICE_NAME)));
        assert!(log.iter().any(|c| c.contains("sing-box.exe")));
    }

    #[test]
    fn status_reports_running_when_schtasks_and_tasklist_say_so() {
        let mgr = temp_manager("status");
        let sys = MockSys::with(|cmd| {
            if cmd.program == "schtasks" && cmd.args.first().map(String::as_str) == Some("/query") {
                CmdOutput {
                    code: Some(0),
