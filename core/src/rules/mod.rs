...
/// first-match semantics), then the RKN-blocked bucket routed through the
/// best available tunnel — a selected Goida server (a full proxy) is
/// preferred over WARP; if neither is set up the bucket is omitted and that
/// traffic falls through to `direct`. Finally the blocks-Russia bucket (if
/// `goida_tag` is set and the community domain list is non-empty). An
/// override into a bucket with no tag set falls back to `direct` — it just
/// has no effect until that engine is set up.
///
/// The RKN-blocked list is what actually holds youtube/discord/etc., so
/// tying it to WARP-only meant selecting a Goida server tunneled nothing
/// (reproduced live: server active, `route.rules` empty, every blocked site
/// still `direct` → connection timeout). Goida is preferred because it's a
/// full outbound proxy that reaches blocked-*and*-throttled services; WARP
/// only helps the throttled-not-blocked case.
pub fn build_rules(
    overrides: &[Override],
    warp_tag: Option<&str>,
    ...
    // RKN-blocked bucket → strongest available tunnel (Goida server first,
    // then WARP). Omitted entirely when no tunnel is set up.
    if let Some(blocked_tag) = goida_tag.or(warp_tag) {
        rules.push(json!({ "rule_set": REFILTER_TAG, "outbound": blocked_tag }));
    }
...
mod tests {
    ...
    fn build_rules_orders_overrides_before_buckets() {
        ...
        // RKN-blocked bucket prefers a selected Goida server over WARP.
        assert_eq!(rules[1]["rule_set"], "refilter_domains");
        assert_eq!(rules[1]["outbound"], "goida");
        assert_eq!(rules[2]["domain_suffix"][0], "blocked.example");
        assert_eq!(rules[2]["outbound"], "goida");
    }

    /// Regression: selecting a Goida server with no WARP and an empty
    /// community list must still route the RKN-blocked bucket through goida —
    /// otherwise `route.rules` is empty and every blocked site goes `direct`
    /// (the live "youtube doesn't work with the engine running" bug).
    #[test]
    fn goida_without_warp_still_routes_rkn_bucket() {
        let rules = build_rules(&[], None, Some("goida"), &[]);
        assert_eq!(rules.len(), 1);
        assert_eq!(rules[0]["rule_set"], "refilter_domains");
        assert_eq!(rules[0]["outbound"], "goida");
    }

    /// With only WARP set up, the RKN-blocked bucket still routes through it.
    #[test]
    fn warp_only_routes_rkn_bucket_through_warp() {
        let rules = build_rules(&[], Some("warp"), None, &[]);
        assert_eq!(rules.len(), 1);
        assert_eq!(rules[0]["rule_set"], "refilter_domains");
        assert_eq!(rules[0]["outbound"], "warp");
    }
