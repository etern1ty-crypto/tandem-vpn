...
fn build_tls(q: &HashMap<String, String>, force: bool) -> Option<Value> {
    ...
    // (confirmed against the real binary: "uTLS is required by reality
    // client"), so this must be unconditional whenever reality is enabled —
    // not just when the source URI carried an `fp` param. `fp` defaults to
    // "chrome" upstream when absent, so do the same here.
    let fp = q.get("fp");
    if fp.is_some() || security == "reality" {
        let fingerprint = normalize_fingerprint(fp.map(String::as_str));
        tls.insert(
            "utls".into(),
...
/// Coerce a source `fp` param to a fingerprint sing-box's uTLS actually
/// accepts. The public subscription lists are full of values the real binary
/// rejects at startup — wrong case (`QQ` vs `qq`), typos (`chromefp`,
/// `hellochrome`), or nonsense (`360`) — and a single bad outbound aborts the
/// whole engine (`FATAL initialize outbound[N]: unknown uTLS fingerprint`).
/// Match case-insensitively against sing-box 1.13's known set and fall back to
/// `chrome` (uTLS's own default) for anything unrecognized.
fn normalize_fingerprint(fp: Option<&str>) -> &'static str {
    // The client-hello IDs bundled in sing-box's uTLS (utls.HelloID names,
    // lowercased). `random`/`randomized` map to uTLS's randomized presets.
    match fp.unwrap_or("chrome").trim().to_ascii_lowercase().as_str() {
        "chrome" => "chrome",
        "firefox" => "firefox",
        "edge" => "edge",
        "safari" => "safari",
...
fn parse_vless(url: &Url, index: usize) -> Option<GoidaConfig> {
    ...
    // sing-box's vless only supports the single flow value `xtls-rprx-vision`
    // (or none). Public lists carry variants it rejects at startup
    // (`xtls-rprx-vision-udp443`, `xtls-rprx-direct`, …), and one such
    // outbound aborts the whole engine (`FATAL … unsupported flow: …`). Emit
    // the field only when it's the one value sing-box accepts; drop the rest
    // (the connection just uses no flow, which is valid).
    if q.get("flow").map(String::as_str) == Some("xtls-rprx-vision") {
...
mod tests {
    ...
    /// Regression test: sing-box 1.13.14 refuses to start ("uTLS is
    /// required by reality client") when a reality outbound has no `utls`
    /// block. Public subscription lists commonly omit `fp` — this must not
    /// silently drop `utls`, or a single such candidate takes down the
    /// entire engine (reproduced live: 5140 Goida candidates, one without
    /// `fp`, config rejected at startup, "0 servers passed" for all of them).
    #[test]
    fn reality_without_fp_still_gets_utls() {
        let uri = "vless://0b08b1c4-342a-4992-ac5f-5b9eff2327a0@20.151.233.69:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=example.com&pbk=abc123&sid=ef#no-fp";
        let cfg = parse_uri(uri, 0).unwrap();
        assert_eq!(cfg.outbound["tls"]["utls"]["enabled"], true);
        assert_eq!(cfg.outbound["tls"]["utls"]["fingerprint"], "chrome");
    }

    /// Regression test: sing-box rejects uTLS fingerprints it doesn't know
    /// (`FATAL … unknown uTLS fingerprint: QQ`). Public lists carry wrong
    /// case (`QQ`), typos (`chromefp`, `hellochrome`) and nonsense (`360`);
    /// one bad outbound aborts the whole engine. `fp` must be coerced to a
    /// value sing-box accepts, falling back to `chrome`.
    #[test]
    fn bad_fingerprint_is_normalized() {
        // Wrong case → lowercased to the valid `qq`.
        let uri = "vless://uuid@a.example.com:443?encryption=none&security=reality&pbk=k&fp=QQ#x";
        let cfg = parse_uri(uri, 0).unwrap();
        assert_eq!(cfg.outbound["tls"]["utls"]["fingerprint"], "qq");

        // Garbage → falls back to chrome instead of being passed through.
        let uri =
            "vless://uuid@a.example.com:443?encryption=none&security=reality&pbk=k&fp=chromefp#x";
        let cfg = parse_uri(uri, 1).unwrap();
        assert_eq!(cfg.outbound["tls"]["utls"]["fingerprint"], "chrome");
    }

    /// Regression test: sing-box's vless only supports the flow value
    /// `xtls-rprx-vision` (`FATAL … unsupported flow: xtls-rprx-vision-udp443`).
    /// Any other flow value from the list must be dropped, not emitted.
    #[test]
    fn unsupported_flow_is_dropped() {
        let uri = "vless://uuid@a.example.com:443?encryption=none&flow=xtls-rprx-vision-udp443&security=reality&pbk=k#x";
        let cfg = parse_uri(uri, 0).unwrap();
        assert!(cfg.outbound.get("flow").is_none());

        // The one supported value is still emitted.
        let uri = "vless://uuid@a.example.com:443?encryption=none&flow=xtls-rprx-vision&security=reality&pbk=k#x";
        let cfg = parse_uri(uri, 1).unwrap();
        assert_eq!(cfg.outbound["flow"], "xtls-rprx-vision");
    }
