...
### Разблокируй YouTube, Discord и Telegram на Windows — в один клик, без сервера и подписки.

Десктопный GUI, который сам управляет лучшими open-source движками обхода DPI
(**Zapret / Flowseal**). Никаких `.bat`-файлов, командной строки и раскопок в конфигах.
Скачал, нажал одну кнопку — готово.
...
[![Скачать](https://img.shields.io/github/v/release/etern1ty-crypto/tandem-vpn?label=Скачать&color=3ddc97)](https://github.com/etern1ty-crypto/tandem-vpn/releases/latest)
[![English](https://img.shields.io/badge/README-English-blue)](README.md)

![tandem-vpn — обход блокировок в один клик](docs/media/hero.gif)

</div>

---

Удобный графический интерфейс (GUI) для управления популярными средствами обхода DPI (в первую
очередь **Zapret / Flowseal**). Больше не нужно вручную качать скрипты, распаковывать архивы и
разбираться с командной строкой.

## ⚡ Быстрый старт (за 3 шага)

1. **Скачать** последнюю версию со страницы [Releases](https://github.com/etern1ty-crypto/tandem-vpn/releases/latest). Установите и запустите приложение (нужны права Администратора).
2. Нажмите кнопку **«Скачать и установить Zapret»**. Приложение само скачает актуальную версию Flowseal и правильно её распакует.
...
![Главное окно](docs/media/screenshot-main.png)

---

## 🛡️ «Антивирус ругается / Windows предупреждает» — прочитайте

Это ожидаемо, и вот честная причина:

- tandem-vpn загружает **драйвер ядра WinDivert** и правит файл **hosts** — это то же
  низкоуровневое поведение, что и у настоящих вирусов, поэтому эвристика антивирусов и
  **SmartScreen** реагируют на **любой** инструмент этого класса (включая сам Zapret).
- У нас **нет телеметрии и нет бэкенда.** Payload Zapret качается напрямую из
  [официальных релизов Flowseal](https://github.com/Flowseal/zapret-discord-youtube/releases).
- **Проверьте сами:** в каждом релизе есть SHA-256 и ссылка на
  [VirusTotal](https://www.virustotal.com/). Исходники открыты (GPL-3.0), сборки CI публичные.

Если появился SmartScreen: **Подробнее → Выполнить в любом случае.** Чтобы антивирус не удалял
драйвер — добавьте папку с программой в исключения.
...
A: Так как программа модифицирует системные настройки сети и использует драйвер WinDivert, некоторые антивирусы могут ошибочно реагировать на неё (False Positive). Добавьте папку с программой в исключения.

**Q: Перестали работать некоторые игры, например с античитами.**
A: Отключите Zapret (кнопка «Удалить службы») на время игры или убедитесь, что включён параметр **Game Filter**.

**Q: Это VPN?**
A: Нет, трафик не идёт через нас. Фаза 1 — это локальный обход DPI; он не скрывает ваш IP. Речь про *доступность* ресурсов, а не про анонимность.
...
> tandem-vpn — инструмент для доступа к правомерной информации и обхода сетевой цензуры.
> Используйте его в соответствии с законами вашей страны.
